...
import { discountCodes, orders, orderItems } from "../../drizzle/schema";
...
export async function checkAndGenerateDiscountCode(customerId: string) {
  ...
  const unusedCode = await db.query.discountCodes.findFirst({
    where: and(
      ...
      eq(discountCodes.customerId, customerId)
    ),
  });
  ...
  const purchaseStats = await getCustomerPurchaseStats(customerId);
 ...
 */
export async function validateDiscountCode(code: string, customerId: string) {
...
export async function previewDiscount(
  ...
  const discount = await validateDiscountCode(code, customerId);
 ...
 */
export async function generateDiscountCodeForCustomer(customerId: string) {
 ...
 */
export async function getAvailableDiscountCodes(customerId: string) {
  const codes = await db.query.discountCodes.findMany({
    where: and(
      eq(discountCodes.isAvailable, true),
      eq(discountCodes.isUsed, false),
 ...
 */
export async function isEligibleForNthOrderDiscount(customerId: string) {
  const orderCountResult = await db
    .select({ count: count() })
    .from(orders)
    .where(eq(orders.customerId, customerId));
